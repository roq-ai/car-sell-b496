...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DealershipInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dealerships` : null),
    () => getDealerships({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Dealerships',
      path: isTenantUser ? `/dealerships/view/${data?.[0]?.id}` : '/dealerships',
      entity: 'dealership',
    ...
    {
      name: 'Car Maintenance Histories',
      path: '/car-maintenance-histories',
      entity: 'car_maintenance_history',
      service: AccessServiceEnum.PROJECT,
      icon: FiTool,
    },
    { name: 'Car Models', path: '/car-models', entity: 'car_model', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
