...
type ColumnType = ColumnDef<CarModelInterface, unknown>;

interface CarModelListPageProps {
...
export function CarModelListPage(props: CarModelListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CarModelInterface>>(
    () => `/car-models?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/car-models/view/${record.id}`} passHref legacyBehavior>
                <Button
