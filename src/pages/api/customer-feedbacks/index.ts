...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getCustomerFeedbacksPublic();
    }
  ...
  async function getCustomerFeedbacksPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'customer_feedback');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.customer_feedback.count(countOptions as unknown),
  ...
  async function getCustomerFeedbacks() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.customer_feedback
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'customer_feedback'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
