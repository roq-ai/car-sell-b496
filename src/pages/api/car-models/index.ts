...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getCarModelsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'car_model');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.car_model.count(countOptions as unknown),
      prisma.car_model.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getCarModels() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.car_model
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'car_model'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
