...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getSalesManagersPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'sales_manager');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.sales_manager.count(countOptions as unknown),
      prisma.sales_manager.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getSalesManagers() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.sales_manager
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'sales_manager'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createSalesManager() {
    ...
    const data = await prisma.sales_manager.create({
      data: body,
    });
