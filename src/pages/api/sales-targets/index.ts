...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getSalesTargetsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'sales_target');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.sales_target.count(countOptions as unknown),
      prisma.sales_target.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getSalesTargets() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.sales_target
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'sales_target'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createSalesTarget() {
    ...
    const data = await prisma.sales_target.create({
      data: body,
    });
