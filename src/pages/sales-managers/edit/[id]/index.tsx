...
import { getSalesManagerById, updateSalesManagerById } from 'apiSdk/sales-managers';
...
function SalesManagerEditPage() {
  ...
  const handleSubmit = async (values: SalesManagerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSalesManagerById(id, values);
      mutate(updated);
      resetForm();
