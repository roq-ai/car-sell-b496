...
type ColumnType = ColumnDef<CustomerFeedbackInterface, unknown>;

interface CustomerFeedbackListPageProps {
...
export function CustomerFeedbackListPage(props: CustomerFeedbackListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CustomerFeedbackInterface>>(
    () => `/customer-feedbacks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CustomerFeedbackInterface) => {
    if (hasAccess('customer_feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/customer-feedbacks/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="customer_feedback" />
        </Flex>

        {hasAccess('customer_feedback', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/customer-feedbacks/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
