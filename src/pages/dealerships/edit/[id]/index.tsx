...
import { getDealershipById, updateDealershipById } from 'apiSdk/dealerships';
...
function DealershipEditPage() {
  ...
  const handleSubmit = async (values: DealershipInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      await imageUploaderRef.current.handleUpload();
      const updated = await updateDealershipById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<DealershipInterface>({
    initialValues: data,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="dealership" entityId={id} />
        </FormWrapper>
