...
import { getCarInsurances } from 'apiSdk/car-insurances';
...
import { PromotionCampaignListPage as ShowCaseList } from 'pages/promotion-campaigns';

function DashboardPage() {
  const { session } = useSession();

  const carInsuranceStat = useSWR<PaginatedInterface<CarInsuranceInterface>>(
    () => `/car-insurances`,
    () => getCarInsurances({ limit: 1, offset: 0 }),
  );

  const carModelStat = useSWR<PaginatedInterface<CarModelInterface>>(
    () => `/car-models`,
    () => getCarModels({ limit: 1, offset: 0 }),
  );

  const purchaseStat = useSWR<PaginatedInterface<PurchaseInterface>>(
    () => `/purchases`,
    () => getPurchases({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Car Insurances',
      result: carInsuranceStat?.data?.totalCount,
      isLoading: carInsuranceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Car Models',
      result: carModelStat?.data?.totalCount,
      isLoading: carModelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Purchases',
      result: purchaseStat?.data?.totalCount,
      isLoading: purchaseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`promotion-campaigns`} passHref legacyBehavior>
                  <Button
