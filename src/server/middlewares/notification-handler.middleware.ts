...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order_created.create': {
    roles: ['business-owner', 'sales-manager', 'sales-representative', 'customer-service-representative'],
    key: 'order_id',
    tenantPath: [],
    userPath: [],
  },
  'support_ticket_opened.create': {
    roles: ['business-owner', 'customer-service-representative'],
    key: 'ticket_id',
    tenantPath: [],
    userPath: [],
  },
  'sales_report_generated.create': {
    roles: ['business-owner', 'sales-manager'],
    key: 'report_id',
    tenantPath: [],
    userPath: [],
  },
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.dealership.findFirst({
